      ...
      <div class="flex bg-white rounded-lg shadow-lg overflow-hidden mx-auto max-w-sm lg:max-w-4xl">
        <div class="w-full p-8 lg:w-1/2">
          <h2 class="text-2xl font-semibold text-gray-700 text-center">
            Pusher Dev Groupchat
          </h2>
          <p class="text-xl text-gray-600 text-center">Create an account</p>
          <a href="#"
             class="flex items-center justify-center mt-4 text-white rounded-lg shadow-md">
            <div class="px-4 py-3">
          ...
          <form @submit.prevent="register">
            ...
            <div class="mt-4">
              <div class="flex justify-between">
                ...
                <a href="#"
                   class="text-xs text-gray-500">Forget Password?</a>
              </div>
          ...
          <div class="mt-4 flex items-center justify-between">
            ...
            <router-link :to="{ name: 'login' }"
                         class="text-xs text-gray-500 uppercase">or sign in</router-link>
            <span class="border-b w-1/5 md:w-1/4"></span>
          </div>
        </div>
...
const register = () => {
  initials.value = form.value.name.match(/(\b\S)?/g).join("").match(/(^\S|\S$)?/g).join("").toUpperCase()
  console.log();
  axios.post(`${import.meta.env.VITE_API_URL}/register`, {
    name: form.value.name,
    email: form.value.email,
    password: form.value.password,
    image: generateAvatar(initials.value, "white", randColor),
  }).then((res) => {
